...
from cpolyagamma import test, digamma, NegBinHS
...
for rho in rhos:
    ...
    if not os.path.exists(dir):
        os.makedirs(dir)
    for file in glob.glob("./simulation/rho_%3.2f/*.pickle"%rho)[0:200]:
        ...
        print("Number of true variables is %d"%sum(opt_gamma))
        ...
        m_eta = np.median(model.get_eta_mat(), axis=0)
        m_gamma = m_eta < 10 # use default 10 as threshold

        # plot ROC and compute AUC
        fpr, tpr, thresholds = roc_curve(opt_gamma, -m_eta)
        roc_auc = auc(fpr, tpr)

        neg_bin_simu = {"seed": ran_seed, "beta": m_beta,
                        "eta": m_eta, "omega": m_omega,
        ...
        fig.savefig(os.path.join(dir, "negbin_hs_seed_%d_%3.2f.pdf"%(ran_seed, rho)), bbox_inches='tight')
        plt.close()
        ...
        tprs[-1][0] = 0.0
        aucs.append(roc_auc)
        pickle.dump(neg_bin_simu, open(os.path.join(dir, "negbin_hs_mcmc_simu_%d_rho_%3.2f.pickle" % (ran_seed, rho)), "wb"), protocol=2)
    ...
    pickle.dump({"tprs": tprs, "aucs": aucs, "times": times, "mccs": mccs, "precisions": precisions, "recalls": recalls, "F1s": F1s}, 
                open(os.path.join(dir, "negbin_hs_rho_%3.2f_summary.pickle"%rho), "wb"), protocol=2)