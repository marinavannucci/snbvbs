...
print(np.nonzero(negbin_dict["opt_gamma"]))
X = negbin_dict["X"]
y = negbin_dict["y"]

model = NegBinSSMCMC(ran_seed)
model.set_data(X, y)
model.set_prior(0.01, 0.01, 2, 20)  # set prior on over-dispersion parameter r
model.set_mcmc(20000, 3000, 1000)
model.run_mcmc(True)

neg_bin_simu = {"beta": model.get_beta_mat(),
                "gamma": model.get_gamma_mat(),
                "omega": model.get_omega_mat(),
                "beta0": model.get_beta0_vec(),
                "tau": model.get_tau_vec(),
                "r": model.get_r_vec(),
                "time": model.get_time()}
pickle.dump(neg_bin_simu, open("negbin_ss_mcmc_simu_%d.pickle" % (ran_seed), "wb"), protocol=2)
